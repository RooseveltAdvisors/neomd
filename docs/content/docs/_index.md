...
- **Out-of-office auto-replies (screened-in only)** — the headless daemon answers incoming mail with a `[ooo]` vacation reply, but **only for senders on your screened-in list** — spam, sales pitches, and unscreened senders never learn you're away (something server-side autoresponders can't do); one reply per sender per OOO period, RFC 3834 loop protection, markdown body rendered exactly like a hand-written neomd email, auto-expires on the `until` date [→](https://neomd.ssp.sh/docs/configuration/headless/#out-of-office-auto-replies)
...
- **Per-email reminders** — open an email and press `H`, enter `+2h`, `tomorrow 09:00`, or a date/time; it moves to Waiting and returns to Inbox when due. No mail is sent; the original stays recoverable in Trash [→](https://neomd.ssp.sh/docs/sending/#reminders)
