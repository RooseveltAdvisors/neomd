...
- Review and stability
  ...
  - **Reliability: `?` no longer trapped in compose textinputs** — the global `?` → help binding fired from any state, including `stateCompose`. Compose feeds keys into a `bubbles/textinput` (To/CC/BCC/Subject), which never received `?` because the global handler intercepted first. Now gated to all states _except_ `stateCompose`, so `?` types verbatim while composing and still toggles help everywhere else
